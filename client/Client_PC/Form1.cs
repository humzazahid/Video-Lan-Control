...
namespace Client_PC
{
        ...
        private void waitForData()
        {
            try
            {
                ...
                while (read.CanRead)
                {
                    ...
                    else if (rcvMsg.Contains("movie"))
                    {
                        axWindowsMediaPlayer1.Visible = true;
                        string rcv = rcvMsg.Remove(0,5);

                        axWindowsMediaPlayer1.URL = rcv;


                    }
                    else if (rcvMsg.Contains("control"))
                    {
                        if(rcvMsg.Contains("play"))
                        {
                            axWindowsMediaPlayer1.Ctlcontrols.play();
                        }
                        else if (rcvMsg.Contains("pause"))
                        {
                            axWindowsMediaPlayer1.Ctlcontrols.pause();
                        }
                        else if (rcvMsg.Contains("stop"))
                        {
                            axWindowsMediaPlayer1.Ctlcontrols.stop();
                        }
                        else if (rcvMsg.Contains("forward"))
                        {
                            axWindowsMediaPlayer1.Ctlcontrols.fastForward();
                        }
                        else if (rcvMsg.Contains("backward"))
                        {
                            axWindowsMediaPlayer1.Ctlcontrols.fastReverse();
                        }
                        else if (rcvMsg.Contains("close"))
                        {
                            axWindowsMediaPlayer1.close();
                        }
                        else if (rcvMsg.Contains("volumeu"))
                        {
                            vol += 20;
                            if(vol > 100)
                            {
                        ...
                        else if (rcvMsg.Contains("volumed"))
                        {
                            vol -= 20;
                            if (vol < 0)
                            {
    ...
    class Recieve_File
    {
        ...
        public void recieve_file(string path)
        {
            ...
            MessageBox.Show("Connection Made\n" + path + "\nFile has been recieved in program debug folder!");
